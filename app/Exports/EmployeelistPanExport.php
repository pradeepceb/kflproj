...
class EmployeelistPanExport implements FromCollection, WithHeadings, WithEvents
{
    protected $data;
    protected $slno;
    protected $Healper;
    ...
    return [
        AfterSheet::class => function (AfterSheet $event) {
            ...
            $event->sheet->getDelegate()->getStyle('A2:J2')
                ...
                ->getStartColor()
                ->setARGB('f0f8ff');
            $event->sheet->mergeCells('A2:J2')->setCellValue('A2', "EMPLOYEES LIST(PAN/ Category Wise)");
            ...
            $event->sheet->setCellValue('B3', "CODE");
            $event->sheet->setCellValue('C3', "EMPLOYEE NAME");
            $event->sheet->setCellValue('D3', "EMPLOYEE CATEGORY");
            $event->sheet->setCellValue('E3', "DESIGNATION");
            $event->sheet->setCellValue('F3', "DEPARTMENT");
            $event->sheet->setCellValue('G3', "ACTIVE TYPE");
            $event->sheet->setCellValue('H3', "DOB");
            $event->sheet->setCellValue('I3', "JOINING DATE");
            $event->sheet->setCellValue('J3', "PAN");
            ...
            $event->sheet->setCellValue('D'.$i, $this->Healper->GetCategory(@$val->catg));
            $event->sheet->setCellValue('E'.$i, $this->Healper->Getdesignation(@$val->desg_code));
            $event->sheet->setCellValue('F'.$i, $this->Healper->GetDepartment(@$val->dept_no));
            $event->sheet->setCellValue('G'.$i, $this->Healper->GetActiveType(@$val->active_type));
            $event->sheet->setCellValue('H'.$i, @$val->DOB);
            $event->sheet->setCellValue('I'.$i, @$val->DOJ);
            $event->sheet->setCellValue('J'.$i, @$val->pan_no);
