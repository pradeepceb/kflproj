                ...
                <div class="info">
                  <div class="row">
                    <div class="col-md-12 text-left p-0">
                      <h6 class="mb-0">LATE ARRIVAL PROCESSING</h6>
                    </div>
                    <div class="col-lg-12 mx-auto">
                      <div style="overflow-x:auto;">
                        <table style="width:250%;" class="table border-none table-hover text-center">
                          <thead class="py-2">
                            <tr class="py-2">
                              <th width="5%">EMPCODE</th>
                              <th width="20%">NAME</th>
                              <th width="5%">MONTH</th>
                              @for ($i=1; $i<=$lateArrivals[0]['month']; $i++)
                                <th width="5%">{{$i}}</th>
                              @endfor
                            </tr>
                          </thead>
                          <tbody>
                            @foreach($lateArrivals as $ky=>$val)
                            <tr>
                              <td width="5%">{{ $val['emp_no'] }}</td>
                              <td width="20%">{{ $val['emp_name'] }}</td>
                              <td width="5%">{{ $val['monthName'] }}</td>
                              @for ($i=1; $i<=$lateArrivals[0]['month']; $i++)
                                <td width="5%">{{$val[$i]}}</td>
                              @endfor
                            </tr>
