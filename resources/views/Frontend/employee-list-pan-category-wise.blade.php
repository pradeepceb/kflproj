...
$time = date("M d, Y h:i A");
    ...
    if(!isset($_GET['pan']) && !isset($_GET['type'])){
        $pan= "all";
        $type= "all";
    } else {
        $pan= $_GET['pan'];
...
@endphp    
                <h5 class="data-heading1 pt-0">The Samaj<span>Report run on: {{ $time }}</span></h5>
                      <h3 class="data-heading p-0">EMPLOYEES LIST(PAN/ Category Wise)</h3>
                                            <div style="text-align: right; margin-top:-35px; padding-bottom:7px;">
                                              
                               <a href="{{ url('/') }}/employee-list-pan-category-wise-print<?php echo $curl; ?>" class="btn btn-primary" target="_blank" id="print_btn" ><i class="fa fa-print"></i> Print</a>

                               <a class="btn btn-primary" target="_blank" id="xl_btn" href="{{ url('/') }}/employee-list-pan-category-wise-xl<?php echo $curl; ?>" >Excel</a>
    ...
    <select name="type" class="col-lg-2 form-control" required>
        ...
        @endforeach
                    ...
                    <select name="pan" class="col-lg-2 form-control" required>
                    <option value="all" @php if($pan=="all"){echo "selected";} @endphp>All</option>
                    <option value="y" @php if($pan=="y"){echo "selected";} @endphp>Yes </option>
                    <option value="n" @php if($pan=="n"){echo "selected";} @endphp>No </option>
                    </select>
                    &nbsp;&nbsp;
                ...
                <select name="active_type" class="col-lg-2 form-control" required>
                    ...
                    <table style="margin-left:10px;">
                        <tr>
                            ...
                            <td><a href="{{ url('/') }}/employee-list-pan-category-wise" class="btn btn-info "><i class="fa fa-refresh" aria-hidden="true"></i></a></td>
                        </tr>
                    </table>
                                                    </form>
                                        </div>
...
<table class="table" id="myTable" width="100%">
    <thead>
        ...
        <tbody> 
            ...
            @if(in_array($v->category_code,$ucatg))
                <tr style="background: aliceblue;">
                    <td colspan="9" align="left" style="padding: 0px; border-bottom: 1px solid rgb(0, 0, 0); ">
        ...
        <tfoot>
    <tr>
        <td align="right" colspan="10">{{ $employee_list->links('paginate.pager5') }} </td>
    </tr>
</tfoot>
</table>
    ...
    <script type="text/javascript">
        function PrintTable() {
            ...
            var style = '<style type = "text/css">';
                ...
                style = style + ".tbh5{ text-decoration: underline; margin-left:20px; margin-top: 5px; }";
                style = style + "</style>";
