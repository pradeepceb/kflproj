...
<h5 class="data-heading1 pt-0">The Samaj<span>Report run on: {{ $time }}</span></h5>
                                            <h3 class="data-heading p-0">EMPLOYEES LIST(Category/ Pay Grade Wise)</h3>
                                            <div style="text-align: right; margin-top:-35px; padding-bottom:7px;">
           <a href="{{ url('/') }}/employee-list-pay-grade-wise-print<?php echo $curl; ?>" class="btn btn-primary" target="_blank" id="print_btn" ><i class="fa fa-print"></i> Print</a>

           <a class="btn btn-primary" target="_blank" id="xl_btn" href="{{ url('/') }}/employee-list-pay-grade-wise-xl<?php echo $curl; ?>" >Excel</a>
...
@endphp         
         <div class="col-xl-12 col-md-12 col-sm-12 col-12" style="padding-right:0px !important;padding-left:0px !important;">
            ...
            <select name="grade" class="col-lg-2 form-control" required>
                <option value="all_grade" @php if($grade=="all_grade"){echo "selected";} @endphp >All</option>
            @foreach ($pay_grade as $val)
                <option value="{{ $val->pay_grade_code }}" @php if($grade==$val->pay_grade_code){echo "selected";} @endphp>
                    @if($val->pay_scale=="NO SCALE")
                    {{ $val->pay_scale }}({{ $val->pay_grade_code }})
                    @else
                    {{ $val->pay_scale }}
                    @endif
                </option>
            ...
            <select name="active_type" class="col-lg-2 form-control" required>
                ...
                <table style="margin-left:10px;">
                    <tr>
                        ...
                        <td><a href="{{ url('/') }}/employee-list-pay-grade-wise" class="btn btn-info "><i class="fa fa-refresh" aria-hidden="true"></i></a></td>
                    </tr>
                </table>
            </form>
                                        </div>
...
@endforeach
                ...
                <tfoot>
                    <tr>
                        <td align="right" colspan="9">{{ $employee_list->links('paginate.pager3') }} </td>
                    </tr>
                </tfoot>
            </table>
